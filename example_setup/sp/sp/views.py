            ...
            context.update(
                {
                    "user_attrs": sorted(
                        [
                            (field.name, getattr(self.request.user, field.name))
                            for field in self.request.user._meta.get_fields()
                            if field.concrete
                        ]
                    ),
                }
...
def custom_update_user(sender, instance, attributes, user_modified, **kargs):
    ...
    for k, v in attributes.items():
        u = set.intersection(set(v), set(["true", "false"]))
        if u:
            setattr(instance, k, u.pop() == "true")
