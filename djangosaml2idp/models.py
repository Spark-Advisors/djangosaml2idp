...
class ServiceProvider(models.Model):
    ...
    dt_updated = models.DateTimeField(
        verbose_name="Updated at", auto_now=True, null=True, blank=True
    )
    ...
    remote_metadata_url = models.CharField(
    ...
    local_metadata = models.TextField(
    ...
    def field_value_changed(self, field_name: str) -> bool:
        ...
        return current_value != getattr(self, "_loaded_db_values", {}).get(
    ...
    def _refresh_from_remote(self) -> bool:
        try:
            self.local_metadata = validate_metadata(
                fetch_metadata(self.remote_metadata_url)
            )
            ...
            if now() > self.metadata_expiration_dt:
                raise ValidationError(
                    f"Remote metadata for SP {self.entity_id} was refreshed, but contains an expired validity datetime."
                )
            return True
        except Exception as e:
            raise ValidationError(
                f"Metadata for SP {self.entity_id} could not be pulled from remote url {self.remote_metadata_url}."
            ) from e
    ...
    def _refresh_from_local(self) -> bool:
        try:
            ...
            if now() > self.metadata_expiration_dt:
                raise ValidationError(
                    f"Local metadata for SP {self.entity_id} contains an expired validity datetime or none at all, no remote metadata found to refresh."
                )
            return True
        except Exception as e:
            # Could not extract a valid expiry timestamp, return False (+ log an error)
            raise ValidationError(
                f"Metadata expiration dt for SP {self.entity_id} could not be extracted from local metadata."
            ) from e
    ...
    def refresh_metadata(self, force_refresh: bool = False) -> bool:
        ...
        if not self.remote_metadata_url and not self.local_metadata:
            logger.error(
                f"Local metadata for SP {self.entity_id} is not present, and no remote metadata found to refresh."
            )
            return False
        ...
        if self.remote_metadata_url:
            try:
                ...
                logger.error(
                    "Unable to refresh remote metadata", extra={"exception": str(e)}
                )
                return False

        if (
            force_refresh
            or (not self.metadata_expiration_dt)
            or (now() > self.metadata_expiration_dt)
            or self.field_value_changed("local_metadata")
        ):
            try:
                return self._refresh_from_local()
            except ValidationError as e:
                logger.error(
                    "Unable to refresh local metadata", extra={"exception": str(e)}
                )
                return False
    ...
    _nameid_field = models.CharField(
    ...
    _sign_response = models.BooleanField(
    ...
    _sign_assertion = models.BooleanField(
    ...
    _signing_algorithm = models.CharField(
        verbose_name="Signing algorithm",
        blank=True,
        null=True,
        max_length=256,
        choices=[(constant, pretty) for (pretty, constant) in xmldsig.SIG_ALLOWED_ALG],
        help_text="If not set, use settings.SAML_AUTHN_SIGN_ALG.",
    )
    _digest_algorithm = models.CharField(
        verbose_name="Digest algorithm",
        blank=True,
        null=True,
        max_length=256,
        choices=[
            (constant, pretty) for (pretty, constant) in xmldsig.DIGEST_ALLOWED_ALG
        ],
        help_text="If not set, default to settings.SAML_AUTHN_DIGEST_ALG.",
    )

    _encrypt_saml_responses = models.BooleanField(
    ...
    def metadata_path(self) -> str:
        ...
        if not os.path.exists(path):
            try:
                ...
                logger.error(
                    f"Could not create temporary folder to store metadata at {path}: {e}"
                )
                raise
        ...
        if (
            ...
            or refreshed_metadata
            or (
                self.dt_updated
                and self.dt_updated.replace(tzinfo=pytz.utc)
                > datetime.datetime.fromtimestamp(os.path.getmtime(filename)).replace(
                    tzinfo=pytz.utc
                )
            )
        ):
...
class PersistentId(models.Model):
    ...
    persistent_id = models.UUIDField(
        "User Persistent Id for this SP", default=uuid.uuid4
    )
