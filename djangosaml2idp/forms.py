...
class ServiceProviderAdminForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "_encrypt_saml_responses": forms.Select(
                choices=boolean_form_select_choices
            ),
    ...
    def clean__attribute_mapping(self):
        ...
        try:
            ...
            raise ValidationError(
                "The provided string could not be parsed with json. ({})".format(e)
            )
    ...
    def clean(self):
        ...
        if not (
            cleaned_data.get("remote_metadata_url")
            or cleaned_data.get("local_metadata")
        ):
            raise ValidationError(
