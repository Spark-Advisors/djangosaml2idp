...
def encode_saml(saml_envelope: str, use_zlib: bool = False) -> bytes:
    ...
    before_base64 = (
        zlib.compress(saml_envelope.encode())[2:-4]
        if use_zlib
        else saml_envelope.encode()
    )
...
def fetch_metadata(remote_metadata_url: str) -> str:
    ...
    try:
        ...
        if content.status_code != 200:
            raise Exception(
                f"Non-successful request, received status code {content.status_code}"
            )
    except Exception as e:
        raise ValidationError(
            f"Could not fetch metadata from {remote_metadata_url}: {e}"
        )
    return content.text
...
def extract_validuntil_from_metadata(metadata: str) -> datetime.datetime:
    ...
    try:
        metadata_expiration_dt = arrow.get(
            ET.fromstring(metadata).attrib["validUntil"]
        ).datetime
    except Exception as e:
        ...
        raise ValidationError(
            f"Could not extra ValidUntil timestamp from metadata: {e}"
        )
