...
class NameIdBuilder:
    ...
    @classmethod
    def _get_nameid_opaque(
        cls, user_id: str, salt: bytes = b"", *args, **kwargs
    ) -> str:
    ...
    def get_nameid_persistent(
        ...
        **kwargs,
        ...
        """
        return str(
            PersistentId.objects.get_or_create(sp=sp, user=user)[0].persistent_id
        )

    @classmethod
    def get_nameid_email(
        cls, user_id: str, user: settings.AUTH_USER_MODEL = None, **kwargs
    ) -> str:
        if "@" not in user_id:
            raise Exception(
...
class BaseProcessor:
    ...
    def get_user_id(
        self, user, name_id_format: str, service_provider: ServiceProvider, idp_config
    ) -> str:
        ...
        return NameIdBuilder.get_nameid(
            user_id, name_id_format, sp=service_provider, user=user
        )

    def create_identity(
        self, user, sp_attribute_mapping: Dict[str, str]
    ) -> Dict[str, str]:
...
def instantiate_processor(
    processor_cls: Type[BaseProcessor], entity_id: str
) -> BaseProcessor:
    try:
    ...
    if not isinstance(processor_instance, BaseProcessor):
        raise ValidationError(
            "{} should be a subclass of djangosaml2idp.processors.BaseProcessor".format(
                processor_cls
            )
        )
