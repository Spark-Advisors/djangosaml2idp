...
def sso_entry(request: HttpRequest, *args, **kwargs) -> HttpResponse:
    ...
    logger.debug(
        "SSO requested to IDP with binding {}".format(request.session["Binding"])
    )
    logger.debug(
        "--- SAML request [\n{}] ---".format(
            repr_saml(request.session["SAMLRequest"], b64=True)
        )
    )
...
def get_sp_config(sp_entity_id: str) -> ServiceProvider:
    ...
    try:
        ...
        raise ImproperlyConfigured(
            _(
                "No active Service Provider object matching the entity_id '{}' found"
            ).format(sp_entity_id)
        )
    return sp
...
def get_authn(req_info=None):
    req_authn_context = (
        req_info.message.requested_authn_context if req_info else PASSWORD
    )
    broker = AuthnBroker()
...
def build_authn_response(user: User, authn, resp_args, service_provider: ServiceProvider) -> list:  # type: ignore
    ...
    idp_server = IDP.load()
    idp_name_id_format_list = idp_server.config.getattr("name_id_format", "idp") or [
...
class IdPHandlerViewMixin:
    ...
    def render_login_html_to_string(self, context=None, request=None, using=None):
        ...
        if custom_login_template_name:
            template = self._fetch_custom_template(
                custom_login_template_name, default_login_template_name, using
            )
    ...
    def _fetch_custom_template(
        custom_name: str, default_name: str, using: Optional[str] = None
    ) -> Template:
    ...
    def create_html_response(
        self, request: HttpRequest, binding, authn_resp, destination, relay_state
    ):
        """Login form for SSO"""
        if binding == BINDING_HTTP_POST:
            ...
            html_response = {
                "data": self.render_login_html_to_string(
    ...
    def render_response(
        self, request: HttpRequest, html_response, processor: BaseProcessor = None
    ) -> HttpResponse:
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            authn_resp = build_authn_response(
                request.user, get_authn(), resp_args, service_provider
            )
...
class SSOInitView(LoginRequiredMixin, IdPHandlerViewMixin, View):
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        authn_resp = build_authn_response(
            request.user, get_authn(), passed_data, service_provider
        )
    ...
    def get(self, request: HttpRequest, *args, **kwargs):
        ...
        try:
            ...
            hinfo = idp_server.apply_binding(
                binding, resp.__str__(), resp.destination, relay_state, response=True
            )
        except Exception as excp:
        ...
        logger.debug(
            "--- {} Response [\n{}] ---".format(
                self.__service_name, repr_saml(resp.__str__().encode())
            )
        )
        logger.debug(
            "--- binding: {} destination:{} relay_state:{} ---".format(
                binding, resp.destination, relay_state
            )
        )
...
def get_multifactor(request: HttpRequest) -> HttpResponse:
    if hasattr(settings, "SAML_IDP_MULTIFACTOR_VIEW"):
        multifactor_class = import_string(
            getattr(settings, "SAML_IDP_MULTIFACTOR_VIEW")
        )
    else:
...
def metadata(request: HttpRequest) -> HttpResponse:
    ...
    """
    return HttpResponse(
        content=IDP.metadata().encode("utf-8"), content_type="text/xml; charset=utf8"
    )
