...
from djangosaml2idp.processors import (
    BaseProcessor,
    NameIdBuilder,
...
class TestBaseProcessor:
    ...
    def test_instantiate_processor_failing_cls(self):
        ...
        with pytest.raises(
            ImproperlyConfigured, match="Failed to instantiate processor: "
        ):
    ...
    def test_instantiate_processor_wrong_cls(self):
        ...
        with pytest.raises(
            ValidationError,
            match="should be a subclass of djangosaml2idp.processors.BaseProcessor",
        ):
    ...
    def test_extract_user_id_default_to_username(self):
        ...
        assert (
            BaseProcessor("entity-id").get_user_id(
                user, NAMEID_FORMAT_UNSPECIFIED, service_provider, idp
            )
            == "test_username"
        )
    ...
    def test_extract_user_id_from_sp_config_if_method(self):
        ...
        service_provider = ServiceProvider(
            entity_id="entity-id", _nameid_field="random_method"
        )
        idp = IDP.load().config
    ...
    def test_identity_dict_creation(self):
        ...
        assert (
            BaseProcessor("entity_id").create_identity(
...
class TestNameIdBuilder:
    def test_get_nameid_opaque(self):
        ...
        assert (
            NameIdBuilder._get_nameid_opaque(user.username)
    ...
    def test_get_nameid_persistent(self, sp_metadata_xml):
        ...
        sp1 = ServiceProvider.objects.create(
            entity_id="sp_entity_id_1", local_metadata=sp_metadata_xml
        )
        sp2 = ServiceProvider.objects.create(
            entity_id="sp_entity_id_2", local_metadata=sp_metadata_xml
        )
    ...
    def test_get_nameid_email_invalid(self):
        ...
        with pytest.raises(
            Exception,
            match=f"user_id {user.username} does not contain the '@' symbol, so is not a valid NameID Email address format.",
        ):
    ...
    def test_request_unmapped_nameid(self):
        ...
        with pytest.raises(
            NotImplementedError,
            match=f"{requested_nameid_format} has not been mapped in NameIdBuilder.format_mappings",
        ):
            NameIdBuilder.get_nameid(
                user.username, nameid_format=requested_nameid_format
            )
    ...
    def test_request_unimplemented_nameid(self):
        ...
        with pytest.raises(
            NotImplementedError,
            match=f"{requested_nameid_format} has not been implemented NameIdBuilder methods",
        ):
            NameIdBuilder.get_nameid(
                user.username, nameid_format=requested_nameid_format
            )
