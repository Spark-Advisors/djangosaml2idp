...
from djangosaml2idp.utils import (
    encode_saml,
...
class TestSAMLEncodeAndDecode:
    ...
    def test_with_minimal_saml_request_b64(self, saml_request_minimal):
        assert repr_saml(encode_saml(saml_request_minimal), b64=True) == self.prettify(
            saml_request_minimal
        )

    def test_with_internal_saml_response_zlib(self, saml_request_minimal):
        assert repr_saml(
            encode_saml(saml_request_minimal, use_zlib=True)
        ) == self.prettify(saml_request_minimal)
...
class TestMetadataValidation:
    ...
    def test_validate_metadata_valid(self):
        md = validate_metadata(
            '<?xml version="1.0" encoding="UTF-8"?><content></content>'
        )
    ...
    def test_extract_validuntil_from_metadata_valid(
        self, settings, sp_metadata_xml, use_tz, tzinfo
    ):
        settings.USE_TZ = use_tz
        valid_until_dt_extracted = extract_validuntil_from_metadata(sp_metadata_xml)
        assert valid_until_dt_extracted == datetime.datetime(
