...
class TestAdminForm:
    ...
    def test_nometadata_given(self):
        ...
        assert (
            "Either a remote metadata URL, or a local metadata xml needs to be provided."
    ...
    def test_invalid_local_metadata(self):
        ...
        assert form.is_valid() is False
        assert (
            "Metadata expiration dt for SP  could not be extracted from local metadata."
            in form.errors["__all__"]
        )
    ...
    @pytest.mark.parametrize("use_tz, tzinfo", [(True, timezone.utc), (False, None)])
    def test_valid_remote_metadata_url(
        self, mock_get, settings, sp_metadata_xml, use_tz, tzinfo
    ):
        settings.USE_TZ = use_tz
    ...
    def test_invalid_remote_metadata_url(self, mock_get):
        ...
        assert form.is_valid() is False
        assert (
            "Metadata for SP  could not be pulled from remote url https://ok."
            in form.errors["__all__"]
        )
    ...
    def test_metadata_invalid_not_json_parseable(self, sp_metadata_xml):
        ...
        assert form.is_valid() is False
        assert (
            "The provided string could not be parsed with json."
            in form.errors["_attribute_mapping"][0]
        )
    ...
    def test_metadata_invalid_nodict(self, sp_metadata_xml):
        ...
        assert form.is_valid() is False
        assert (
            "The provided attribute_mapping should be a string representing a dict."
            in form.errors["_attribute_mapping"][0]
        )
    ...
    def test_metadata_invalid_dict_wroncontent(self, sp_metadata_xml):
        ...
        assert form.is_valid() is False
        assert (
            "The provided attribute_mapping should be a dict with strings for both all keys and values."
            in form.errors["_attribute_mapping"][0]
        )
