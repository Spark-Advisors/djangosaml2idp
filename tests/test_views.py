    ...
    return {
        ...
        "service": {
            "sp": {
                ...
                "idp": {"test_generic_idp": {}},
            }
        },
        "metadata": {"local": ["tests/xml/metadata/idp_metadata.xml"]},
    }
...
def saml_login_request_factory(sp_conf_dict):
    def _factory(binding: str = BINDING_HTTP_REDIRECT) -> str:
        ...
        if binding == BINDING_HTTP_REDIRECT:
            ...
            return parse.parse_qs(parse.urlparse(result["headers"][0][1]).query)[
                "SAMLRequest"
            ][0]
...
def sp_testing_configs() -> dict:
    return {
        "test_sp_with_no_processor": {"attribute_mapping": {}},
...
def saml_logout_request_factory():
    def _factory(
        id="Request ID", format=saml.NAMEID_FORMAT_UNSPECIFIED, name_id="user1"
    ):
...
class TestStoreParamsInSession:
    def test_works_correctly_with_get(self, saml_request_minimal, sample_get_request):
        ...
        assert all(
            item in sample_get_request.session.items()
...
class TestSSOEntry:
    ...
    def test_sso_entry_returns_bad_request_if_no_samlrequest(
        self, saml_request_minimal, sample_get_request
    ):
...
class TestIdPHandlerViewMixin:
    ...
    def test_render_login_html_to_string_renders_default_if_custom_not_found(
        ...
        mock_get_template.assert_called_once_with(
            "djangosaml2idp/login.html", using=None
        )

    def test_render_login_html_to_string_renders_result_of_fetch_custom_template(
        self, mock_get_template, mocker
    ):
        fetch_custom_mock = mocker.patch(
            "djangosaml2idp.views.IdPHandlerViewMixin._fetch_custom_template"
        )
    ...
    @pytest.mark.django_db
    def test_set_sp_works_if_sp_defined(
        self, settings, sp_metadata_xml, sp_testing_configs
    ):
        ServiceProvider.objects.create(
            entity_id="test_generic_sp", local_metadata=sp_metadata_xml
        )
        ...
        assert (
            ...
            == sp_testing_configs["test_generic_sp"]["attribute_mapping"]
        )

    @pytest.mark.django_db
    def test_set_processor_errors_if_processor_cannot_be_loaded(self, sp_metadata_xml):
        ServiceProvider.objects.create(
            entity_id="test_sp_with_bad_processor",
    ...
    def test_set_processor_defaults_to_base_processor(self, sp_metadata_xml):
        ServiceProvider.objects.create(
            entity_id="test_sp_with_no_processor",
            local_metadata=sp_metadata_xml,
            _attribute_mapping="{}",
        )
    ...
    def test_render_response_constructs_request_session_properly(self, sp_metadata_xml):
        ...
        mixin.render_response(
            request, html_response, get_sp_config("test_generic_sp").processor
        )
...
class TestLoginProcessView:
    ...
    @pytest.mark.django_db
    def test_goes_through_normally_redirect(
        self, sp_metadata_xml, saml_login_request_factory, logged_in_request
    ):
        ServiceProvider.objects.create(
            entity_id="test_generic_sp", local_metadata=sp_metadata_xml
        )
    ...
    @pytest.mark.django_db
    def test_goes_through_normally_post(
        self, sp_metadata_xml, saml_login_request_factory, logged_in_request
    ):
        ServiceProvider.objects.create(
            entity_id="test_generic_sp", local_metadata=sp_metadata_xml
...
class TestLogoutProcessView:
    @pytest.mark.django_db
    def test_slo_view_works_properly_redirect(
        self, sp_metadata_xml, logged_in_request, saml_logout_request_factory
    ):
        ServiceProvider.objects.create(
            entity_id="test_generic_sp", local_metadata=sp_metadata_xml
        )
